...
reduced_counts = np.zeros((mlen, plen, 8))
...
#    _ = m.df.add_edgemask(m.df.mfs, mval, 60) # Insert new edge mask
    for j in range(len(m.df.sfs)):
        pixels, counts, expos = m.cluster_halos(m.df.sfs[j], m.df.mfs[j], pix_sizes, include=True)
        reduced_pixels[i,:,:] += pixels
        reduced_counts[i,:,:] += counts
        reduced_expos[i,:,:] += expos
        print(reduced_counts[i,:4,:])

print(reduced_counts)
suffix = '_' + str(enum) + '_' + str(ccdnum)
sys.exit()
# np.save(f'./data/minos3/channel_expos_1e' + suffix, reduced_expos)
# np.save(f'./data/minos3/channel_count_1e' + suffix, reduced_counts)
# np.save(f'./data/minos3/channel_pixel_1e' + suffix, reduced_pixels)
